...
import numpy as np
from Agent.doublenet import DoubleDQNetwork
...
t_power = []
u_demand = []
doubledqn_episode_rewards = []    #list of all episode rewards. We will plot this against number of episodes. This should be increasing as episodes increase.
...
class DoubleDQNAgent:
    ...
    def __init__(self, env, config):
        ...
        #self._tau = 0.1
        self._ddqn = DoubleDQNetwork(self._sess, env.dim_state, env.dim_action, config.lr)
        self._dir_mod_full = '{0}/{1}-doubledqn'.format(config.dir_mod, config.run_id)
        dir_sum_full = '{0}/{1}-doubledqn'.format(config.dir_sum, config.run_id)
        self._dir_log_full = '{0}/{1}-{2}.txt'.format(config.dir_log, config.run_id, 'doubledqn')
    ...
    def predict(self, state):
        q_value = self._ddqn.predict([state])[0]
        self._ep_maxq = np.max(q_value)
       # print("q_value for Double:",q_value)
    ...
    def work(self):
        ...
        global e_power
        global doubledqn_episode_rewards
        sum_total_power = 0
        sum_user_demand = 0
        demand_counter = 0
        e_power.clear()
...
#                print("\n\nnext-state: ", state_next,power, reward, done,'\n\n')
                ...
                #Here We are Calling Double Deep Q Network Trainer
               # self._train_batch((state, action, reward, state_next, done))
                self._train_batch2((state, action, reward, state_next, done))
                self.flage_train=True
               # print("\n\nstate-> ",state,"\nstate_next-> ", state_next)
                self._write_log(state, state_next)
                state = state_next
                if done:
                    doubledqn_episode_rewards.append(epi_reward)
                    break
        ...
        if (sum_total_power != 0):
            ...
            t_power.append((mean_total_power - 3))
            #t_power.append(self._env._P_MAX)
            #u_demand.append(self._env._DM_MAX/1.e6)
            u_demand.append(mean_user_demand)
            #for j in range(len(total_power)):
             #   print ('power', total_power[j])
        #----------------------
        return t_power, u_demand, doubledqn_episode_rewards
#Here is the code for Double Deep Q network

    def printWeights(self,nm):
        self._ddqn.printWeights(nm)
    ...
    def _train_batch(self, sample):
        ...
        q_values = self._ddqn.predict_target(batch_state_next)
        # q_values = self._ddqn.predict(batch_state_next)
        batch_y = []
        ...
        for q, reward, done in zip(q_values, batch_reward, batch_done):
            if done:
               ...
               q_target = reward + self._GAMMA * np.max(q)
               batch_y.append(reward + self._GAMMA * q_target - 1000)
        _, loss = self._ddqn.train(batch_state, batch_action, batch_y)
        self._ep_loss.append(loss)

        self._ddqn.update_target()

        return True
    
    def _train_batch2(self, sample):
        self._replay_buffer.add_samples([sample])
        ...
        q_values = self._ddqn.predict(batch_state_next) #q values for next state from theta
        best_actions= np.argmax(q_values, axis = 1) #axis = 1 because we want to take argmax over rows
        q_values_target= self._ddqn.predict_target(batch_state_next) #q values for next state from theta_prime
        # q_values = self._ddqn.predict(batch_state_next)
        batch_y = []

        for q_target, best_action, reward, done in zip(q_values_target, best_actions,batch_reward, batch_done):
            if done:
                batch_y.append(reward)
            else:
                batch_y.append(reward + self._GAMMA * q_target[best_action])
        _, loss = self._ddqn.train(batch_state, batch_action, batch_y)
        self._ep_loss.append(loss)
...
#            print('totla',self._max_ep_sts)        
         ...
         #   print("DoubleQN Episode:", self._ep, " Power:",power['drl'])
    ...
    def _load(self, dir_mod, load_id):
        self._saver.restore(self._sess, tf.train.latest_checkpoint(dir_mod + '/' + load_id + '-doubledqn'))
