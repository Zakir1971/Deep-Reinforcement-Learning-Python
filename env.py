...
# =============================================================================
    def state(self):
         ...
         #print("dm = ", dm)
         #print("state_rrh = ", self._state_rrh)
...
#    def state(self):
#         dm = (self._demand - self._DM_MIN) / (self._DM_MAX - self._DM_MIN)
#         #ch_gains = self._get_gains(self._num_rrh, self._num_usr)
#        #print("Demand: ", dm)
#         #print("state_rrh = ", self._state_rrh)
#         #since channel_gains is a 2D matrix, we need to convert it into a 1D matrix. Therefore, we flatten it.
#         return np.concatenate([self._state_rrh, dm, np.unique(np.array(self.ch_gains))], axis=0)
#         #return np.concatenate([self._state_rrh, dm, np.unique(np.array(ch_gains))], axis=0)####Concatenation refers to joining. This function is used to join two or more arrays of the same shape along a specified axis
#         #return np.concatenate([self._state_rrh, dm])  ####Concatenation refers to joining. This function is used to join two or more arrays of the same shape along a specified axis
# =============================================================================
...
# =============================================================================
#    def reset_ch_gain(self):
#         ch_gains = self._get_gains(self._num_rrh, self._num_usr)
#         return ch_gains
# =============================================================================
    
    def reset(self):
        #self.ch_gains = self._get_gains(self._num_rrh, self._num_usr)
        self.reset_channel()
        self.reset_demand()
